...
> **在初始化gitalk的过程中出现了403错误**，发现是gitalk说大家使用某一链接太多次而失效了，最好自己部署gitalk配置中的proxy，部署方法在gitalk的issue中有提到，大概步骤是注册cloudflare账号，创建一个worker，把https://github.com/Zibri/cloudflare-cors-anywhere该链接中的文件所有扔进去。这里我用的是从issues里面抄的一段网友的代码。
>
